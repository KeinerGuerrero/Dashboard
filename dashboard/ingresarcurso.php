<?php 
    include("conexion1.php");
    $con=conectar();


    $sql="SELECT *  FROM cursos";
    $query=mysqli_query($con,$sql);
    $row=mysqli_fetch_array($query);



    
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <title> INGRESAR DATOS</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link href="css/style.css" rel="stylesheet">
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-giJF6kkoqNQ00vy+HMDP7azOuL0xtbfIcaT9wjKHr8RbDVddVHyTfAAsrekwKmP1" crossorigin="anonymous"> 
        
    </head>
    
<body style="background-color: #F1F1F1;">

    <h1 align="center" > 
        <p> 
        <img src="http://1.bp.blogspot.com/-7Z1AXu6m1qk/VZ1aYZht_FI/AAAAAAAAADM/71nPaZw9KeM/s1600/fce11d3e-05be-425e-b540-56f856b2c7ef.gif"width="150" height="150">
       </p>INGRESAR DATOS
    </h1>

<!-- <div class="col-md-5" style="align-content: right;">
 -->
 <div style="width: 50%; margin-left: 25%;background-color:; ">
 <form action="insertarcurso.php" method="POST">

                                    <input type="text" class="form-control mb3" name="id_curso" placeholder="ID CURSO"style="border: 3px solid; color: black;"><br>
                                    <input type="text" class="form-control mb3" name="nom_curso" placeholder="NOMBRE CURSO"style="border: 3px solid; color: black;"><br>
                                    <label for="id_ano_acad"></label>         
                                        <select class="form-control mb-3" name="id_ano_acad" style="border: 3px solid; color: black;" id="id_ano_acad">
                                        <option value="">Id Año Academico</option>
                                        <option value="1">2021</option>
                                        <option value="2">2022</option>
                                        <option value="3">2023</option>
                                        <option value="4">2024</option>
                                        <option value="5">2025</option>
                                    </select>
                                    <label for="id_doc"></label>         
                                        <select class="form-control mb-3" name="id_doc" style="border: 3px solid; color: black;" id="id_doc">
                                        <option value="">Id Docente</option>
                                        <option value="1">Martha Liliana Alonso</option>
                                        <option value="2">Ana Beatriz Vanegas</option>
                                    </select>

                                  <input type="submit" class="btn btn-primary btn-block" value="INGRESAR">
                    </form>
                    <th><label for="busqueda"></label><br><a href="cursos.php" class="btn btn-info">VOLVER</a></th>
                        </div>